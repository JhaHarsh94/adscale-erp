...
export const markRead = asyncHandler(async (req, res) => {
  ...
  io.to(`user:${user?.id}`).emit("notification:updated", { unreadCount: await prisma.notification.count({ where: { userId: user?.id, isRead: false } }) });
...
export const markAllRead = asyncHandler(async (req, res) => {
  ...
  io.to(`user:${user?.id}`).emit("notification:updated", { unreadCount: 0 });
...
export async function createNotification(data: { userId: string; title: string; message: string; type?: string; link?: string }) {
  ...
  io.to(`user:${data.userId}`).emit("notification:updated", { unreadCount: await prisma.notification.count({ where: { userId: data.userId, isRead: false } }) });
  return notification;
}
